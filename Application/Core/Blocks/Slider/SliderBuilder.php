...
namespace Application\Core\Blocks\Slider;
...
class SliderBuilder
{
    ...
    public function getTemplate($id, $settings, $userSliderId=null){

        $sliderId = is_null($userSliderId) ? $this->getSliderByRubricIdAction($id) : $userSliderId;

        //$sliderId = 1;
        $pathToTemplate = '../Application/Core/Blocks/Slider/templates/template'.$sliderId;



        $styleFile = $pathToTemplate."/style.css";
        $htmlFile = $pathToTemplate."/index.html";
        $UniqueStyleBuilder = 'Application\Core\Blocks\Slider\templates\template'.$sliderId.'\SliderTemplate'.$sliderId;
        ...
        if($styleFile) {
            ...
            $obj = $this->setUniqueStyle($style, $htmlString, $jsString, $UniqueStyleBuilder, $settings, $id);
