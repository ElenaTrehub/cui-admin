...
namespace Application\Core\Blocks\About\templates\template3;


use Application\Core\Settings\Settings;
use Application\Services\UtilsService;

class AboutTemplate3
{
    public $utilsService;
    public $settings;
    ...
    public function setUniqueStyle($styleString, $htmlString, $jsString, $settings, $id, $pageName){
        ...
        if($pageName){
            ...
            if(strpos($obj->style, '/*page*/',0)!==false){
                $obj->style = $this->utilsService->parseStyle($obj->style, '/*page*/', '.about-'.$pageName);
            }
            if(strpos($obj->js, '/*page*/',0)!==false){
                $obj->js = $this->utilsService->parseStyle($obj->js, '/*page*/', '.about-'.$pageName);
            }
    ...
    public function setColorsForChildInLightBlock($obj, $colors, $bgColor){

        if($bgColor !== $colors->secondBg){
            if(strpos($obj->style, '/*a_top_bg*/',0)!==false){
                $obj->style = $this->utilsService->parseStyle($obj->style, '/*a_top_bg*/', 'color: '.$colors->thirdBg.';');
            }
            if(strpos($obj->style, '/*a_bottom_bg*/',0)!==false){
                $obj->style = $this->utilsService->parseStyle($obj->style, '/*a_bottom_bg*/', 'color: #cccccc;');
            }
        }
        else{
            if(strpos($obj->style, '/*a_top_bg*/',0)!==false){
                $obj->style = $this->utilsService->parseStyle($obj->style, '/*a_top_bg*/', 'color: '.$colors->mainBg.';');
            }
            if(strpos($obj->style, '/*a_bottom_bg*/',0)!==false){
                $obj->style = $this->utilsService->parseStyle($obj->style, '/*a_bottom_bg*/', 'color: #ffffff;');
            }
        }
    ...
    public function setColorsForChildInDarkBlock($obj, $colors){
        if(strpos($obj->style, '/*a_top_bg*/',0)!==false){
            $obj->style = $this->utilsService->parseStyle($obj->style, '/*a_top_bg*/', 'color: '.$colors->secondBg.';');
        }
        if(strpos($obj->style, '/*a_bottom_bg*/',0)!==false){
            $obj->style = $this->utilsService->parseStyle($obj->style, '/*a_bottom_bg*/', 'color: #cccccc;');
        }
    ...
    public function setColorStyle($obj, $colors){
        ...
        if(isset($obj->set->lastSectionColor)) {
            ...
            if ($obj->set->lastSectionColor === 'light') {
                ...
                $this->setColorsForChildInLightBlock($obj, $colors, $bgAbout[$index]);
    ...
    public function setLightColorStyle($obj, $colors){
        ...
        $this->setColorsForChildInLightBlock($obj, $colors, $obj->set->lastSectionColor);
