...
namespace Application\Core\Blocks\Feature\templates\template2;
...
class FeatureTemplate2
{
    ...
    public function setColorsForChildInDarkBlock($obj, $colors){
        ...
        if(strpos($obj->style, '/*i_f_c*/',0)!==false){
            $obj->style = $this->utilsService->parseStyle($obj->style, '/*i_f_c*/', 'color: '.$colors->thirdBg.';');
        }
    }

    public function setLightBg($obj, $colors, $id){
        if(strpos($obj->style, '/*f_bg*/',0)!==false){
            $obj->style = $this->utilsService->parseStyle($obj->style, '/*f_bg*/', 'background-image: url("../images/'.$this->settings->getPhotoFolderName($id).'/feature_bg.jpg"); background-repeat: no-repeat; background-size: cover; background-position: center center; background-blend-mode: lighten; background-color: rgba(255, 255, 255, .65);');
        }
    }
