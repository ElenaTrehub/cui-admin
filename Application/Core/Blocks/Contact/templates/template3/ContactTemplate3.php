...
namespace Application\Core\Blocks\Contact\templates\template3;
...
class ContactTemplate3
{
    ...
    public function setJs($obj){
        ...
        if(in_array('getMask', $obj->set->libs)){
            ...
            $obj->libs = $obj->libs.$this->jsLibs->getJsLib('getMask');
            if(strpos($obj->js, '//js_code_contact',0)!==false){
                $obj->js = $this->utilsService->parseStyle($obj->js, '//js_code_contact', 'mask(\'[name="phone"]\');');
            }
        }
    ...
    public function setColorsForChildInLightBlock($obj, $colors){
        ...
        if(strpos($obj->style, '/*contact_span_border*/',0)!==false){
            $obj->style = $this->utilsService->parseStyle($obj->style, '/*contact_span_border*/', 'border-bottom: 1px solid #888888;');

        }
        if(strpos($obj->style, '/*contact_icon_c*/',0)!==false){
            $obj->style = $this->utilsService->parseStyle($obj->style, '/*contact_icon_c*/', 'color: '.$colors->titleColor.';');
    ...
    public function setColorsForChildInDarkBlock($obj, $colors){
        ...
        if(strpos($obj->style, '/*contact_span_border*/',0)!==false){
            $obj->style = $this->utilsService->parseStyle($obj->style, '/*contact_span_border*/', 'border-bottom: 1px solid #cccccc;');

        }
        if(strpos($obj->style, '/*contact_icon_c*/',0)!==false){
            $obj->style = $this->utilsService->parseStyle($obj->style, '/*contact_icon_c*/', 'color: '.$colors->anyTextColor.';');
